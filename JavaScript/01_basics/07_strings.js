...
//When we declare a string, it gets stored as an array of individual characters, each having a key (index) value, starting from 0, implyig that strings are iterable. Each character / value can be accessed using key/index. lastName[1] or lastName.charAt(1), both these ways are used to get the character at a specific position. Also each string object has a length property. Also each object has various methods like: 


console.log(firstName.length); //returns total number of characters in the string
console.log(lastName.charAt(1)); //gives the character at specified index position
console.log(lastName.charCodeAt(1)); //gives the ASCII value of character present at specified index.
console.log(lastName.toUpperCase()); //it doesn't change the original string, just returns a new string with all alphabetical characters in upper case.
...
//.replace() replaces only the first occurence of the specified substring by the specified substring. To replace all occurences, either use replaceAll() or use regular expression with a global flag (instead of wrapping substring in '', wrap it inside / /g, ex- replace(/hello/g, "Hi")), replaces hello with Hi
...
//Remember that strings are immutable in languages like Python, JavaScript and Java. But they're mutable in C/C++.
//So, it means that once a string is created, its content cannot be modified. Even if you attempt to modify nth character of a string, the original string will remain unchanged, though program will not through an error. The string modification gets ignored