...
console.log(marvel_heros) //push() only adds an element at the end of parent array. push() can input any kind of data (element/array/object) and just add it as an element at the end of parent array.
marvel_heros.pop()
...
//Syntax: arr.flat(depth), depth is the level to which you want to flattn your array. If we dont give any arguments, then it flattens the array to 1 level. If we're not sure about depth, we can pass Infinity also. It returns a new array, and doesn't modify the original array.
...
console.log(Array.from(str)); //string converted to array
console.log(Array.isArray(Array.from(str))); //true


//Array.from() can convert objects also to an array, but you have to explicitly specify what you want to store in array, keys of object or values of object.
obj = {name: "Suyash", age: 23}
console.log(Array.from(obj)); //empty array []
console.log(Array.from(Object.values(obj))); //non empty array.
